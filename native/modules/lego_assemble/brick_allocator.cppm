export module lego_assemble.brick_allocator;
...
export std::tuple<BrickId, pxr::SdfPath>
allocateBrick(const pxr::UsdStageRefPtr &stage,
	...
	if (!nextIdAttr->SetDefaultValue(pxr::VtValue(nextId + 1))) {
		log_fatal("allocateBrick: Failed to update next_id.");
	}
...
export bool deallocateBrick(const pxr::UsdStageRefPtr &stage, EnvId env_id,
                            BrickId brick_id) {
	auto layer = stage->GetEditTarget().GetLayer();
	pxr::SdfChangeBlock _changes;
	...
	if (brick) {
		...
		if (parent) {
			...
			log_warn("deallocateBrick: Brick prim has no parent!");
		}
	}
	return false;
}
...
export void deallocateAllBricksInEnv(const pxr::UsdStageRefPtr &stage,
                                     EnvId env_id) {
	auto layer = stage->GetEditTarget().GetLayer();
	pxr::SdfChangeBlock _changes;
