...
template <PartLike P> struct PhysicsPartWrapper : SimplePartWrapper<P> {
	template <class... Args>
	explicit PhysicsPartWrapper(
	    std::initializer_list<InterfaceShapePair> if_shapes, Args &&...args)
	    : PhysicsPartWrapper<P>(std::forward<Args>(args)...),
...
struct PhysicsConnectionSegmentWrapper : SimpleWrapper<ConnectionSegment> {
	...
	explicit PhysicsConnectionSegmentWrapper(Args &&...args)
	    : PhysicsConnectionSegmentWrapper(std::forward<Args>(args)...) {}
};

struct PhysicsConnectionBundleWrapper : SimpleWrapper<ConnectionBundle> {
...
template <class PEKs>
using AugmentedPEKs = PEKs::template prepend<physx::PxRigidActor *>;
template <class PEKHs>
using AugmentedPEKHs =
    PEKHs::template prepend<std::hash<physx::PxRigidActor *>>;
template <class PEKEqs>
using AugmentedPEKEqs = PEKEqs::template prepend<std::equal_to<>>;

export template <class Ps, class PEKs = type_list<>, class PEKHs = type_list<>,
                 class PEKEqs = type_list<>, class CSEKs = type_list<>,
                 class CSEKHs = type_list<>, class CSEKEqs = type_list<>>
class PhysicsLegoGraph final
    : public LegoGraph<Ps, PhysicsPartWrapper, pmr_vector_storage,
                       AugmentedPEKs<PEKs>, AugmentedPEKHs<PEKHs>,
                       AugmentedPEKEqs<PEKEqs>, PhysicsConnectionSegmentWrapper,
                       CSEKs, CSEKHs, CSEKEqs, PhysicsConnectionBundleWrapper> {
  public:
	using Base =
	    LegoGraph<Ps, PhysicsPartWrapper, pmr_vector_storage,
	              AugmentedPEKs<PEKs>, AugmentedPEKHs<PEKHs>,
	              AugmentedPEKEqs<PEKEqs>, PhysicsConnectionSegmentWrapper,
	              CSEKs, CSEKHs, CSEKEqs, PhysicsConnectionBundleWrapper>;
	using Self =
	    PhysicsLegoGraph<Ps, PEKs, PEKHs, PEKEqs, CSEKs, CSEKHs, CSEKEqs>;
  ...
  protected:
	class PhysxBinding : private PxSimulationFilterCallbackProxy,
	  ...
	  public:
		explicit PhysxBinding(Self *graph, physx::PxScene *px_scene)
		    : graph_{graph}, px_scene_{px_scene} {
			// Register callbacks
		...
		virtual physx::PxFilterFlags pairFound(
			...
			if (graph_->contact_exclusions_.contains(exclusion_pair)) {
				return physx::PxFilterFlag::eKILL;
			}

			// Enable contact pose report collision between parts
			bool is_part0 =
			    graph_->parts().template alive<physx::PxRigidActor *>(rb0);
			bool is_part1 =
			    graph_->parts().template alive<physx::PxRigidActor *>(rb1);
		...
		virtual void onContact(const physx::PxContactPairHeader &header,
			...
			for (physx::PxU32 i = 0; i < nbPairs; i++) {
				...
				const InterfaceSpec *if0 =
				    graph_->shape_mapping_.template find<ActorShapePair>(
				        {rb0, shape0});
  ...
  private:
		...
		for (const auto &[if_id, shape] : pw.interface_shapes()) {
			if (shape == nullptr) {
				throw std::runtime_error(std::format(
				    "Interface {} of part id {} has null PxShape", if_id, pid));
			}
			if (!shape_mapping_.emplace(
			        {{pid, if_id}, {px_actor, shape}},
			        get_interface_at(pw.wrapped(), if_id))) {
				throw std::runtime_error(std::format(
				    "Failed to map shape to interface {} of part id {}", if_id,
				    pid));
			}
		}
	}
	...
	void on_part_removing(PartId pid, PhysicsPartWrapper<P> &pw) {
		// Update shape <-> interface mapping
		for (const auto &[if_id, shape] : pw.interface_shapes()) {
			if (!shape_mapping_.erase_by_key<InterfaceRef>({pid, if_id})) {
				throw std::runtime_error(std::format(
	...
	void on_connected(ConnSegId csid, const ConnSegRef &csref,
		...
		if (auto stud_shape_mapping =
		        shape_mapping_.project<InterfaceRef, ActorShapePair>(
		            stud_if_ref)) {
		...
		if (csw.px_stud_shape != NullActorShapePair &&
			...
			auto [_, inserted] = contact_exclusions_.emplace(csw.px_stud_shape,
			                                                 csw.px_hole_shape);
			if (!inserted) {
				throw std::runtime_error(std::format(
				    "Failed to add contact exclusion for connection segment {}",
				    csid));
			}
		}
	}
	...
	void on_bundle_created(const ConnectionEndpoint &ep,
		...
		cbw.px_constraint = create_constraint(a_id, b_id, T_a_b);

		// Update skip graph
		if (!skip_graph_.connect(a_id, b_id)) {
			throw std::runtime_error(
			    std::format("Failed to connect parts {} and {} in skip graph "
	...
	void on_bundle_removing(const ConnectionEndpoint &ep,
		...
		destroy_constraint(cbw.px_constraint);
		cbw.px_constraint = nullptr;

		// Update skip graph
		if (!skip_graph_.disconnect(a_id, b_id)) {
			throw std::runtime_error(
