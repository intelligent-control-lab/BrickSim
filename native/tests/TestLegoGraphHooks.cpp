...
using G = LegoGraph<PartList<CustomPart>, SimplePartWrapper, pmr_vector_storage,
                    type_list<>, type_list<>, type_list<>,
                    SimpleWrapper<ConnectionSegment>, type_list<>, type_list<>,
                    type_list<>, SimpleWrapper<ConnectionBundle>,
...
struct Hooks {
	...
	void on_part_added(PartId pid, SimplePartWrapper<P> &pw) {
		...
		added_pw_matches_store =
		    (&pw == g->parts().template get<SimplePartWrapper<P>>(pid));
	}

	template <class P>
	void on_part_removing(PartId pid, SimplePartWrapper<P> &pw) {
		++removing_calls;
		removing_alive_in_store = g->parts().alive(pid);
...
static void test_added_hook() {
	...
	auto ifs0 = std::initializer_list<InterfaceSpec>{
	    mk_stud(10, 2, 2), mk_stud(12, 2, 2), mk_hole(20, 2, 2),
	    mk_hole(22, 2, 2)};
	assert(g.add_part<CustomPart>(std::tuple<>{}, 0.1, BrickColor{255, 0, 0},
	                              ifs0));
	...
	auto ifs1 = std::initializer_list<InterfaceSpec>{
	    mk_stud(11, 2, 2), mk_stud(13, 2, 2), mk_hole(21, 2, 2),
	    mk_hole(23, 2, 2)};
	assert(g.add_part<CustomPart>(std::tuple<>{}, 0.2, BrickColor{0, 255, 0},
	                              ifs1));
	assert(hooks.added_calls == 2);

	auto ifs2 = std::initializer_list<InterfaceSpec>{mk_stud(30, 1, 1),
	                                                 mk_hole(31, 1, 1)};
	assert(g.add_part<CustomPart>(std::tuple<>{}, 0.3, BrickColor{0, 0, 255},
	                              ifs2));
...
static void test_connected_hook() {
	...
	assert(g.connect(ab_ref.first, ab_ref.second, std::tuple<>{}, cs));
	assert(hooks.connected_calls == 1);
	const ConnSegId *stored_csid =
	    g.connection_segments().project<ConnSegRef, ConnSegId>(ab_ref);
...
static void test_bundle_created_hook() {
	...
	assert(g.connect(IR(0, 12), IR(1, 23), std::tuple<>{}, cs));
	assert(hooks.bundle_created_calls == 1);
}
