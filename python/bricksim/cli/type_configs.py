...
BRICKSIM_SOURCE_CHECK_INCLUDE = "python/bricksim/**"
...
def plan_operations(paths: TargetPaths) -> tuple[list[Operation], str, str]:
    ...
    ty_source_includes = []
    if (paths.root / "python" / "bricksim").is_dir():
        ty_source_includes.append(BRICKSIM_SOURCE_CHECK_INCLUDE)
    ty_config_text = render_ty_config(
        python_path=_active_python_environment_path(paths.root),
        ty_roots=ty_roots,
        ty_extra_paths=ty_extra_paths,
        source_includes=ty_source_includes,
    )
