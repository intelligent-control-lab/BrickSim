...
static InterfaceSpec stud(InterfaceId id) {
    return {.id = id, .type = InterfaceType::Stud, .L = 2, .W = 2, .pose = SE3d{}.identity()};
}
static InterfaceSpec hole(InterfaceId id) {
    return {.id = id, .type = InterfaceType::Hole, .L = 2, .W = 2, .pose = SE3d{}.identity()};
}
...
int main() {
    ...
    std::initializer_list<InterfaceSpec> aifs{stud(10), stud(12), hole(20), hole(22)};
    std::initializer_list<InterfaceSpec> bifs{stud(11), stud(13), hole(21), hole(23)};
    ...
    assert(g.connect(InterfaceRef{0, 12}, InterfaceRef{1, 23}, std::tuple<>{}, cs));
    return 0;
}
