export module lego_assemble.brick_specs;

import std;

namespace lego_assemble {

export using BrickUnit = std::int32_t;

export constexpr double BrickLength = 0.008;        // 8 mm per stud
export constexpr double PlateHeight = 0.0032;       // 3.2 mm per plate
export constexpr BrickUnit BrickHeightPerPlate = 3; // 1 brick height = 3 plates
export constexpr double StudDiameter = 0.0048;      // 4.8 mm stud diameter
export constexpr double StudHeight = 0.0017;        // 1.7 mm stud height

export constexpr std::array<double, 3>
brickDimensionsToMeters(const std::array<BrickUnit, 3> &dimensions) {
	return {dimensions[0] * BrickLength, dimensions[1] * BrickLength,
	        dimensions[2] * PlateHeight};
}

export constexpr double
brickMassInKg(const std::array<BrickUnit, 3> &dimensions) {
	// Experimental formula to fit mass of arbitrary bricks
	auto [L, W, H] = dimensions;
	int bricks = H / 3;
	int rem = H % 3;
	double m_brick = 0.1523 * (L * W) + 0.2498 * (L + W) - 0.2485;
	double m_plate = 0.10937 * (L * W) + 0.05671 * (L + W) - 0.02207;
	return (bricks * m_brick + rem * m_plate) / 1000;
}

export using BrickColor = std::array<std::uint8_t, 3>; // RGB

export enum BrickOrientation : std::int8_t {
	DEG_0 = 0,
	DEG_90 = 1,
	DEG_180 = 2,
	DEG_270 = 3,
};

} // namespace lego_assemble
