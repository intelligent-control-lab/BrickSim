...
template <class V, class Less = std::less<>, class Eq = std::equal_to<>>
...
namespace std {
template <class V, class Less, class Eq>
    requires std::is_same_v<Eq, std::equal_to<>> &&
...
struct hash<lego_assemble::UnorderedPair<V, Less, Eq>> {
	...
	size_t operator()(const lego_assemble::UnorderedPair<V, Less, Eq> &p) const
