...
template <typename Scalar_, int Rows_, int Cols_, int Options_, int MaxRows_,
          int MaxCols_>
class Matrix;
template <typename _Scalar, int _Options> class Quaternion;
template <typename _Scalar, int _Dim, int _Mode, int _Options> class Transform;
} // namespace Eigen

PXR_NAMESPACE_OPEN_SCOPE
// clang-format off
class GfMatrix2f; class GfMatrix2d;
class GfMatrix3f; class GfMatrix3d;
class GfMatrix4f; class GfMatrix4d;
class GfVec2i;  class GfVec2f;  class GfVec2d;
class GfVec3i;  class GfVec3f;  class GfVec3d;
class GfVec4i;  class GfVec4f;  class GfVec4d;
class GfQuatf;  class GfQuatd;
class GfTransform;
// clang-format on
...
// ============================================================================
// Matrix/Vector traits & concepts
// ============================================================================

template <class T, class = void> struct mat_traits {
...
template <mat_like T> using mat_scalar_t = typename mat_traits<T>::scalar_type;

template <mat_like T>
inline constexpr std::size_t mat_rows_v = mat_traits<T>::rows;

template <mat_like T>
inline constexpr std::size_t mat_cols_v = mat_traits<T>::cols;

// ── std::array adapters ──────────────────────────────────────────────────────
// Vector: std::array<S,N> ==> Nx1
template <class S, std::size_t N> struct mat_traits<std::array<S, N>> {
...
template <class S, std::size_t R, std::size_t C>
...
template <class S, int R, int C, int Opt, int MR, int MC>
...
// ── PhysX adapters ───────────────────────────────────────────────────────────
template <template <class> class PxVec, class S>
...
template <class S> struct mat_traits<physx::PxMat33T<S>> {
	static constexpr bool valid = true;
	using scalar_type = S;
...
template <class S> struct mat_traits<physx::PxMat34T<S>> {
	static constexpr bool valid = true;
	using scalar_type = S;
...
template <class S> struct mat_traits<physx::PxMat44T<S>> {
	static constexpr bool valid = true;
	using scalar_type = S;
...
template <class Tag, class S, std::size_t R, std::size_t C, class = void>
struct mat_of; // primary

// std::array
template <class S, std::size_t N> struct mat_of<std_tag, S, N, 1, void> {
...
template <> struct mat_of<gf_tag, int, 2, 1, void> {
	using type = pxr::GfVec2i;
};
template <> struct mat_of<gf_tag, float, 2, 1, void> {
	using type = pxr::GfVec2f;
};
template <> struct mat_of<gf_tag, double, 2, 1, void> {
	using type = pxr::GfVec2d;
};
template <> struct mat_of<gf_tag, int, 3, 1, void> {
	using type = pxr::GfVec3i;
};
template <> struct mat_of<gf_tag, float, 3, 1, void> {
	using type = pxr::GfVec3f;
};
template <> struct mat_of<gf_tag, double, 3, 1, void> {
	using type = pxr::GfVec3d;
};
template <> struct mat_of<gf_tag, int, 4, 1, void> {
	using type = pxr::GfVec4i;
};
template <> struct mat_of<gf_tag, float, 4, 1, void> {
	using type = pxr::GfVec4f;
};
template <> struct mat_of<gf_tag, double, 4, 1, void> {
	using type = pxr::GfVec4d;
};
template <> struct mat_of<gf_tag, float, 2, 2, void> {
	using type = pxr::GfMatrix2f;
};
template <> struct mat_of<gf_tag, double, 2, 2, void> {
	using type = pxr::GfMatrix2d;
};
template <> struct mat_of<gf_tag, float, 3, 3, void> {
	using type = pxr::GfMatrix3f;
};
template <> struct mat_of<gf_tag, double, 3, 3, void> {
	using type = pxr::GfMatrix3d;
};
template <> struct mat_of<gf_tag, float, 4, 4, void> {
	using type = pxr::GfMatrix4f;
};
template <> struct mat_of<gf_tag, double, 4, 4, void> {
	using type = pxr::GfMatrix4d;
};

// PhysX
template <class S> struct mat_of<physx_tag, S, 2, 1, void> {
	using type = physx::PxVec2T<S>;
};
template <class S> struct mat_of<physx_tag, S, 3, 1, void> {
	using type = physx::PxVec3T<S>;
};
template <class S> struct mat_of<physx_tag, S, 4, 1, void> {
	using type = physx::PxVec4T<S>;
};
template <class S> struct mat_of<physx_tag, S, 3, 3, void> {
	using type = physx::PxMat33T<S>;
};
template <class S> struct mat_of<physx_tag, S, 3, 4, void> {
	using type = physx::PxMat34T<S>;
};
template <class S> struct mat_of<physx_tag, S, 4, 4, void> {
	using type = physx::PxMat44T<S>;
};

template <class Tag, class S, std::size_t R, std::size_t C>
...
template <mat_like To, mat_like From>
...
template <class Tag, class S, std::size_t R, std::size_t C, mat_like From>
...
template <class Tag, class S, std::size_t N, mat_like From>
...
template <class S, std::size_t N, mat_like From>
    requires(mat_rows_v<From> == N && mat_cols_v<From> == 1)
[[nodiscard]] constexpr std::array<S, N> as_array(const From &src) {
	return as<std::array<S, N>>(src);
}

template <class S, std::size_t R, std::size_t C, mat_like From>
    requires(mat_rows_v<From> == R && mat_cols_v<From> == C)
...
template <class T, class = void> struct quat_traits {
	static constexpr bool valid = false;
};
...
// Eigen Quaternion<S>
template <class S, int Opt> struct quat_traits<Eigen::Quaternion<S, Opt>> {
	static constexpr bool valid = true;
	using scalar_type = S;
...
template <class Tag, class S, class = void> struct quat_of; // primary

template <class S> struct quat_of<eigen_tag, S, void> {
	using type = Eigen::Quaternion<S, 0>;
};

template <class S> struct quat_of<physx_tag, S, void> {
	using type = physx::PxQuatT<S>;
};

template <> struct quat_of<gf_tag, float, void> {
	using type = pxr::GfQuatf;
};

template <> struct quat_of<gf_tag, double, void> {
...
template <quat_like To, quat_like From>
...
template <class S, quat_like From>
...
template <class Tag, class S, quat_like From>
...
template <quat_like To, mat_like From>
...
template <mat_like To, quat_like From>
...
template <class T, class = void> struct transform_traits {
	static constexpr bool valid = false;
};
...
template <class S> struct transform_traits<physx::PxTransformT<S>> {
	static constexpr bool valid = true;
	using scalar_type = S;
...
template <class T>
    requires std::same_as<remove_cvref_t<T>, pxr::GfTransform>
struct transform_traits<T> {
	static constexpr bool valid = true;
	...
	static T make_rt(const R &R33, const V &t) {
		...
		using M4 = decltype(std::declval<const T &>().GetMatrix());
		T out;
		out.SetMatrix(as<M4>(M)); // use your matrix bridge
		return out;
	}
...
template <class S, int Mode, int Opt>
...
template <class Tag, class S, class = void> struct transform_of;

template <class S> struct transform_of<std_tag, S, void> {
	using type = std::array<std::array<S, 4>, 4>;
};
template <class S> struct transform_of<eigen_tag, S, void> {
	using type = Eigen::Matrix<S, 4, 4, 0, 4, 4>;
};
template <class S> struct transform_of<physx_tag, S, void> {
	using type = physx::PxTransformT<S>;
};
template <class S> struct transform_of<gf_tag, S, void> {
...
template <transform_like To, mat_like From>
...
template <transform_like To, mat_like R, mat_like V>
...
template <transform_like To, quat_like Q, mat_like V>
    requires(!mat_like<To>) &&
            (mat_traits<V>::rows == 3 && mat_traits<V>::cols == 1)
...
template <mat_like To, transform_like From>
...
template <quat_like To, transform_like From>
[[nodiscard]] constexpr To as(const From &X) {
	auto R = transform_traits<From>::template rotation33<
	    std::array<std::array<double, 3>, 3>>(X);
	return as<To>(R);
...
template <class Tag, class S, transform_like From>
