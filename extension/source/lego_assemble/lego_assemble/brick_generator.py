...
def create_brick(stage: Usd.Stage, path: str, length_studs=4, width_studs=2, height_plates=3, color=(0.3, 0.3, 0.3)):
    ...
    brick.GetPrim().CreateAttribute("lego_length", Sdf.ValueTypeNames.Int).Set(length_studs)
    brick.GetPrim().CreateAttribute("lego_width", Sdf.ValueTypeNames.Int).Set(width_studs)
    brick.GetPrim().CreateAttribute("lego_height", Sdf.ValueTypeNames.Int).Set(height_plates)
