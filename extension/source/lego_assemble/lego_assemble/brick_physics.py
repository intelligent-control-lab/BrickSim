...
def handle_assembly_contacts():
    ...
    for contact in contact_headers:
        if contact.type == ContactEventType.CONTACT_LOST:
            continue
        actor0: str = intToSdfPath(contact.actor0).pathString
        actor1: str = intToSdfPath(contact.actor1).pathString
        prim0 = stage.GetPrimAtPath(actor0)
        prim1 = stage.GetPrimAtPath(actor1)
        ...
        if not dim_attr0.IsValid() or not dim_attr1.IsValid():
            continue
        dim0 = np.array(dim_attr0.Get(), dtype=int)
        dim1 = np.array(dim_attr1.Get(), dtype=int)

        pose0 = get_rigidbody_transform(actor0)
        pose1 = get_rigidbody_transform(actor1)
